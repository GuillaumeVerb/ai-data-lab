...
summary: "Public MVP: triage, extraction, automation score, human review. Tool use, a single agent."
format: lab
...
A demonstrable local MVP: cockpit UI (input, timeline, artifacts), persisted runs and feedback, explainability panel. Architecture: one agent, tool use, not an MCP server.

## Failure / limit

No published tool-use correctness or false-autonomy rate. The automation score is an internal heuristic. No live Gmail/Slack integration in the README.

## Next

Measure tool-use correctness and the false-autonomy rate. MCP (connectors, servers) stays a Learning node on “exploring”.
