...
import { getWriting, listLabs, listProjects, listWriting } from "@/lib/content";
...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    listWriting(locale).map((item) => ({ locale, slug: item.content_id })),
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const item = getWriting(locale, slug);
  if (!item) return {};
...
export default async function WritingDetailPage({ params }: Props) {
  ...
  const relatedProjects = listProjects(locale).filter(
    (project) =>
      ...
      project.related_writing_ids.includes(item.content_id),
  );
